...
export default {
    ...
    execute: async (message: Message, args: string[]) => {
        ...
        if (args.length !== 2) {
            await message.reply(`Please provide both the current position and the target position (e.g., ${configHandler.getGuildSetting(message.guild, 'COMMAND_PREFIX', 'string')}move 3 1).`);
            return;
        }
